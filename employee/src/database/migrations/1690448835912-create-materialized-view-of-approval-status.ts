...
export class CreateMaterializedViewOfApprovalStatus1690448835912
  implements MigrationInterface
{
  async up(queryRunner: QueryRunner): Promise<void> {
    //create dblink to join relationship of user and approver
    await queryRunner.query(`
        DROP EXTENSION IF EXISTS dblink CASCADE;
        DROP SERVER IF EXISTS authentication CASCADE;
        CREATE EXTENSION IF NOT EXISTS dblink;
        DROP USER MAPPING IF EXISTS FOR ${process.env.DB_USERNAME} SERVER authentication;
        CREATE SERVER IF NOT EXISTS authentication FOREIGN DATA WRAPPER dblink_fdw  OPTIONS (host 'localhost' ,dbname '${process.env.AUTHENTICATION_DB}' , port '${process.env.DB_PORT}');
        CREATE USER MAPPING FOR ${process.env.DB_USERNAME} SERVER authentication OPTIONS ( user '${process.env.DB_USERNAME}' ,password '${process.env.DB_PASSWORD}');
    `);

    //create materialized view of approval status
    await queryRunner.query(`
        ...
        CROSS JOIN payroll_benefit_master pbm 
        INNER JOIN payroll_benefit pb 
        ON pb.payroll_benefit_master_id = pbm.id 
        INNER JOIN employee e ON e.id = pb.employee_id 
        ...
        WHERE workflow.request_type = 'PAYROLL_BENEFIT' AND approval.entity_id = pbm.id
        UNION
        SELECT approval.*, workflow.request_type,workflow.description,raw.id 
        AS request_approval_workflow_id,creator.id 
        AS user_id, creator.username, first_approver_user.username 
        ...
        INNER JOIN company_structure cs5 ON cs5.id = ep.company_structure_position_id  
        INNER JOIN company_structure_component csc5 ON csc5.id = cs5.company_structure_component_id
        WHERE workflow.request_type = 'EMPLOYEE_RESIGNATIon' AND er.id = approval.entity_id
        UNION
        SELECT approval.*, workflow.request_type,workflow.description,raw.id 
        AS request_approval_workflow_id,creator.id 
        AS user_id, creator.username, first_approver_user.username 
